...
export class SideNavComponent implements OnInit {
  ...
  public REST_API_SERVER: string = `${environment.REST_API_URL}/`
  ...
  getData() {
    ...
    const stores = this.authenticationService.getStores();

    if (stores && Array.isArray(stores) && stores.length > 0) {
      const imagePath = stores[0].image;
      this.imageUrl = `${this.REST_API_SERVER}user-logo/${imagePath}`;
      console.log('imageUrl::', this.imageUrl);
    } else {
      console.log('No stores data available or data is not in expected format');
    }
  }
}
